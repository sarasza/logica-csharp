/* Leia quatro números (N1, N2, N3, N4), cada um deles com uma casa decimal, correspondente às quatro notas de um aluno. Calcule a média com pesos 2, 3, 4 e 1,
   ...
   digitada. Recalcule a média (some a pontuação do exame com a média anteriormente calculada e divida por 2). e imprima a mensagem "Aluno aprovado." (caso a média final
   seja 5.0 ou mais ) ou "Aluno reprovado.", (caso a média tenha ficado 4.9 ou menos). Para estes dois casos (aprovado ou reprovado após ter pego exame) apresente na última
...
namespace PrimeiroProjeto
{
    class Program
    {
        ...
        {
            float media, notaExam, mediaFinal;
            ...
            float N4 = float.Parse(valores[3], CultureInfo.InvariantCulture);
            ...
            else if (media < 5.0)
            {
                ...
                notaExam = float.Parse(Console.ReadLine(), CultureInfo.InvariantCulture);
                mediaFinal = ((media + notaExam) / 2);
                Console.WriteLine("Nota do exame: " + notaExam.ToString("F1"), CultureInfo.InvariantCulture);
